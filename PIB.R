...
ggplot(alto_dat,aes(x=Años,y=log_PIB))+geom_point()

#**************************Regresiones Log-Lineales**************************
library("ggplot2")

reg_Alto<-lm(log_PIB~Años,data=alto_dat)
summary(reg_Alto)

residuos_Alto<-summary(reg_Alto)["residuals"]

ggplot(alto_dat,aes(x=Años,y=log_PIB))+geom_point()+stat_smooth(method=lm)
...
reg_Alto_pol1<-lm(log_PIB~Años,data=alto_polig1)
summary(reg_Alto_pol1)
ggplot(alto_polig1,aes(x=Años,y=log_PIB))+geom_point()+stat_smooth(method=lm)

reg_Alto_pol2<-lm(log_PIB~Años,data=alto_polig2)
summary(reg_Alto_pol2)
ggplot(alto_polig2,aes(x=Años,y=log_PIB))+geom_point()+stat_smooth(method=lm)

#Graficos de Regresio LOG-POLIGONAL
ggplot(datos_etapa,aes(x=Años,y=log_PIB,colour=Etapa))+geom_point()+stat_smooth(method = lm)
